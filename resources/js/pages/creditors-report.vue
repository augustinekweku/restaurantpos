<template>
    <div class=" my-3 reports-page animate__animated animate__fadeIn mb-5">
        <div class="row">
            <div class="col-sm-3">
                ...
                <div class="text-center">
                    ...
                    <Button
                        class="m-2"
                        @click="getCreditorClearedOrders"
                        type="primary"
                        >Show all</Button
                    >
                </div>
                ...
                <Select @on-change="fetchCreditorItem" not-found-text="No items available" class="py-2" v-model="item_id" placeholder="Select Item" width="40px">
                    <Option
                    ...
                    <Button
                        class="m-2"
                        @click="filterDateForCreditorItem"
                        type="primary"
                        >Filter Item</Button
                    >
                    <Button
                        class="m-2"
                        @click="getItemsForCreditorReport"
                        type="primary"
        ...
        <div v-if="allRecordsTable" class="reports-table px-2 m-1 mb-5 pb-2" id="print-block">
            <h3 class="text-center my-1">Credit</h3>
            <div
...
export default {
    ...
    methods: {
        async fetchCreditorItem(){
            console.log(this.item_id)
            const getResults = await this.callApi(
                "get",
                `app/fetch_creditor_item/${this.item_id}`
            );
        ...
        async filterDateForCreditorItem(){
            if (this.fromDate.trim() == "")
                return this.error("Beginning date is required");
            if (this.toDate.trim() == "")
                return this.error("End date name is required");
            if (!this.item_id) {
                return this.error("Item is not specified")
            }
            const getResults = await this.callApi(
                "get",
                `app/get_date_range_for_creditor_item/${this.fromDate}/${this.toDate}/${this.item_id}`
            );
        ...
        dateFn(RowObj){
            //return RowObj.order
            let d = new Date( RowObj.creditor_order.created_at );
        ...
        onRowDoubleClick(params) {
            ...
            let obj = {
                ...
                items: params.row.creditor_order_details,
        ...
        itemsFn(RowObj) {
            //return RowObj.creditor_order_details.length
            let items = [];
            let reducedItems = [];
            if (RowObj.creditor_order_details.length > 2) {
                for (let j = 0; j < 2; j++) {
                    reducedItems.push(
                        " " +
                            RowObj.creditor_order_details[j].item_name +
                            " X " +
                            RowObj.creditor_order_details[j].quantity
                    );
                }
                return `<span style="font-size:13px !important">${reducedItems} ...</span>`;
            }
            if (RowObj.creditor_order_details.length < 3) {
                for (let i = 0; i < RowObj.creditor_order_details.length; i++) {
                    items.push(
                        " " +
                            RowObj.creditor_order_details[i].item_name +
                            " X " +
                            RowObj.creditor_order_details[i].quantity
                    );
                }
        ...
        async filterDate() {
            ...
            const getResults = await this.callApi(
                "get",
                `app/get_cleared_creditor_date_range/${this.fromDate}/${this.toDate}`
            );
        ...
        async getItemsForCreditorReport() {
            const getItemsForCreditorReport = await this.callApi(
                "get",
                "app/get_items_for_creditor_report"
            );
            if ((getItemsForCreditorReport.status = 201)) {
                this.item_reports = getItemsForCreditorReport.data.data;
                this.total_amount = getItemsForCreditorReport.data.total_amount;
                this.total_quantity = getItemsForCreditorReport.data.total_quantity;
                this.showRangeResults = false
        ...
        async getCreditorClearedOrders() {
            const getCreditorClearedOrders = await this.callApi(
                "get",
                "app/get_cleared_creditor_orders"
            );
            if ((getCreditorClearedOrders.status = 201)) {
                this.reports = getCreditorClearedOrders.data.reports;
                this.reportsTotal = getCreditorClearedOrders.data.total;
                this.showAllResults = true
                this.showRangeResults = false
                this.fromDate = ""
                this.toDate = ""
                //console.log(getCreditorClearedOrders.data);
            } else {
                this.swr();
            }
        },
    ...
    created() {
        this.getCreditorClearedOrders();
        this.getItemsForCreditorReport();
        this.getAllItems();
