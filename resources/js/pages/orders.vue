...
export default {
    ...
    methods:{
        ...
        closeModal(){
            ...
            async list(page=1){
            await axios.get(`/app/get_requested_orders?page=${page}`).then(({data})=>{
                this.Orders = data
            }).catch(({ response })=>{
                console.error(response)
            })
        ...
        async getRequestedOrders(){
            const getRequestedOrders = await this.callApi("get", "app/get_requested_orders");
            if (getRequestedOrders.status == 200) {
                this.Orders = getRequestedOrders.data;
            } else {
                this.swr();
            }
        }
